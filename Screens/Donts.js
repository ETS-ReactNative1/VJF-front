...
function Donts(props) {
	...
	useEffect(() => {
		async function loadDonts() {
			var rawResponse = await fetch(
				`https://vite-jai-faim.herokuapp.com/users/myDonts/${token}`
			)
		...
		try {
			...
			if (manualIngredient != '') {
				const listDont = await fetch(
					`https://vite-jai-faim.herokuapp.com/users/adddonts/${token}`,
					{
						method: 'POST',
	...
	async function handleDontDelete(dont) {
		...
		var rawResponse = await fetch(
			`https://vite-jai-faim.herokuapp.com/users/deletedonts/${token}/${dont}`,
			{
				method: 'DELETE',
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
				body: `dont=${dont}`,
